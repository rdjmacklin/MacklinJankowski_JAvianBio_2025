...
With these files, the user should have all they need to conduct the full analysis. Note that this script will take a long time (i.e. days to weeks depending on processing power), at times running in parallel. The script is set to use all but one of the available cores for parallel processing. Users may adjust this option by modifying the `cores` object within the script. As the bootstrapping process relies on randomly generated seeds generated in the script, users wishing to exactly replicate our analysis should use the commands in this script commented out on lines 335-338 to import the seeds we used from a saved file rather than randomly generate new seeds themselves. The file containing our sequence of seeds can be found in `Data/Cleaned/Seeds`. 
