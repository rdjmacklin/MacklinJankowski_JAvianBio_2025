...
* The user will need to provide the eBird Status and Trend range polygons for CBCH and BCCH. We accessed these from the 2021 eBird Status and Trends release (Fink et al. 2022). This version of the data is archived, and so users must contact <ebird@cornell.edu> to request the version needed to exactly replicate our analysis. The two `.gpkg` files should be placed within the `Data/Raw/eBird-Status-and-Trends` directory.
